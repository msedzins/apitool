...
UI-015's four approved visual states are the collection pane active, keyboard help overlay, request pane active, and response pane active. Their canonical 100×30 render baselines will be stored once under `testdata/ui-015/` as `collection-focus.txt`, `keyboard-help.txt`, `request-focus.txt`, and `response-focus.txt`; snapshot tests read these files directly. The design specification defines behavior and layout intent, while those fixture files are the exact rendered-screen source of truth.
