...
func TestAppendLogOmitsCallerSuppliedHostFields(t *testing.T) {
	workspace := t.TempDir()
	store, err := runtime.Open(workspace)
	if err != nil {
		t.Fatal(err)
	...
	key := runtime.Key{CollectionPath: "payments", Environment: "prod", RequestID: "users/list"}
	for name, entry := range map[string]runtime.LogEntry{
		"opaque host":        {Key: key, Method: "GET", Host: "eyJhbGciOiJub25lIn0.eyJzdWIiOiJhIn0.c2lnbmF0dXJl"},
		...
		"oauth opaque value": {Key: key, Method: "GET", OAuthEndpointHost: "c2lnbmF0dXJl.eyJzdWIiOiJhIn0.eyJhbGciOiJub25lIn0"},
	} {
		t.Run(name, func(t *testing.T) {
			if err := store.AppendLog(entry); err != nil {
				t.Fatalf("AppendLog() error = %v, want caller host fields omitted", err)
			}
		})
	}
	raw, err := os.ReadFile(filepath.Join(workspace, ".apitool", "logs", "executions.jsonl"))
	...
	for _, unsafe := range [][]byte{[]byte("eyJhbGciOiJub25lIn0.eyJzdWIiOiJhIn0.c2lnbmF0dXJl"), []byte("query-token-456"), []byte("client-secret-789"), []byte("c2lnbmF0dXJl.eyJzdWIiOiJhIn0.eyJhbGciOiJub25lIn0")} {
		if bytes.Contains(raw, unsafe) {
			t.Errorf("log contains unsafe host input %q: %s", unsafe, raw)
		}
	}
	if bytes.Contains(raw, []byte(`"host":`)) || bytes.Contains(raw, []byte(`"oauth_endpoint_host":`)) {
		t.Errorf("log = %s, want caller-supplied host fields omitted", raw)
	}
}
